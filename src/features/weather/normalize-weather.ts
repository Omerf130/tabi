...
export type ConditionLabelResolver = (
  code: number,
...
function normalizeCondition(
  condition: WeatherApiCondition | undefined,
  resolveConditionLabel: ConditionLabelResolver,
): WeatherCondition {
...
function normalizeHourSummary(
  hour: WeatherApiHour,
  resolveConditionLabel: ConditionLabelResolver,
): WeatherHourSummary | null {
  if (!hour.time) {
    return null;
  }
  ...
  return {
    ...
    condition: normalizeCondition(hour.condition, resolveConditionLabel),
    isDay: hour.is_day === 1,
  };
}
...
function normalizeHourlyFromForecast(
  ...
  for (const day of forecastDays.slice(0, 2)) {
    for (const hour of day.hour ?? []) {
      const normalized = normalizeHourSummary(hour, resolveConditionLabel);
      if (normalized) {
...
function normalizeDaySummary(
  ...
  return {
    ...
    condition: normalizeCondition(day.day.condition, resolveConditionLabel),
    chanceOfRainPercent:
...
export function normalizeWeatherSnapshot(
  ...
  const normalizedDays = forecastDays
    ...
    .map((day) => normalizeDaySummary(day, resolveConditionLabel))
  ...
  const current: WeatherCurrent = {
    ...
    condition: normalizeCondition(currentPayload.condition, resolveConditionLabel),
  ...
  return {
    ...
    today,
    forecast,
    hourly: normalizeHourlyFromForecast(forecastDays, resolveConditionLabel),
