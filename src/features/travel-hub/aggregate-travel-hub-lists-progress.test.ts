...
import { createHebrewListsTranslator } from "@/features/i18n/test-translators";
import { aggregateTravelHubListsProgress } from "./aggregate-travel-hub-lists-progress";

const formatProgressLabel = (values: { completed: number; total: number }) =>
  createHebrewListsTranslator()("progressLabel", values);
...
describe("aggregateTravelHubListsProgress", () => {
  ...
  it("returns null progress label when there are no list items", () => {
    const progress = aggregateTravelHubListsProgress([], formatProgressLabel);
