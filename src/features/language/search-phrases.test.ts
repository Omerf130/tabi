...
import { createHebrewLanguageTranslator } from "@/features/i18n/test-translators";
...
describe("searchPhrases", () => {
  const t = createHebrewLanguageTranslator();
  const getCategoryLabel = createPhrasebookCategoryLabelResolver(t);
  const phrases = toSearchablePhrases(getDefaultPhrasebookPack().phrases, getCategoryLabel);
  ...
  it("builds a normalized search blob", () => {
    ...
    expect(buildSearchBlob(phrase!, getCategoryLabel)).toContain("קוניצ");
  });
